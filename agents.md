...
- **Smooth Scroll Contract**: The page is scrolled by Lenis. `<html>` carries no `scroll-smooth`: that class makes the browser animate every programmatic scroll, against Lenis' own animation. Anything that moves the page programmatically takes the instance from `useLenisInstance()` (`src/components/shared/LenisProvider.tsx`) and calls `lenis.scrollTo`. A native `scrollTo`/`scrollIntoView` with `behavior: "smooth"` runs its own animation against Lenis and the two fight the whole way down.
...
- **Complex HUD Logic**: `src/components/shared/TelemetryNav.tsx` (Visible nav bar, observer-driven active state, velocity trace sampled on a timer rather than per frame. The terminal button asks for attention: 14s after load it pings and shows a callout for 9s, then keeps a blinking dot. Opening the terminal stores `orbital.terminal.opened` in `localStorage`, and the beacon never shows again.)
...
- **Target Reticle**: `src/components/shared/TargetReticle.tsx` (Bounding-box lock on `data-hud-target` elements, in two modes. Fine pointer: brackets follow the cursor and the innermost target under it locks. Touch: scrolling drives the lock. The outermost visible target crossing 45% of the viewport height locks, its brackets clamp into the viewport, and the label shows in a fixed bottom-left readout. Every card-level box on the page carries a target (`CORE.0N`, `PROJ.0N`, `PROJ.0N.SCHEMATIC`, `ARCH.0N`, `JRNY.0N`, `CERT.0N`, `COMMS.*`); tag new cards the same way. Never tag something that moves on its own, like the badge marquee: the reticle re-measures only on scroll.)
...
- **Launch Sequence**: `src/components/shared/SystemBootSequence.tsx` (Boot screen as SELF_TEST → NAV_LOCK → IGNITION → WARP, stages keyed by progress in `bootSequence.ts`). Its `WarpField.tsx` is a forward-flight starfield where cruise and warp are the same code at different speeds, and `NavGlobe.tsx` spins a `sphere.ts` wireframe by writing `d` through refs from a rAF loop, never through React state. The sequence plays on every load: the owner wants visitors to watch the site set itself up. After one full run, `localStorage` key `orbital.boot.seen` unlocks a `SKIP_SEQUENCE` button plus Esc, Enter and Space. A skip jumps to the warp handover, never to a hard cut.
- **Case Studies**: `src/app/projects/[slug]/page.tsx`, statically generated from `src/lib/data/caseStudies.ts` (keyed by `TopProject.slug`). Every claim must trace to the project's public README, source or live site, and each entry lists its `sources`. Qader is a company product the owner works on as an employee, not his to disclose: its page and timeline entry give the general shape and the tools only, with no module counts, internal features, customers, figures or unreleased work, and never a claim that he owns the code or started it. A project with no entry gets no page and no CASE_STUDY button; never pad one with guesses. Card code excerpts (`TopProject.code`) are verbatim, contiguous hunks from a named commit. A project with private source has no `code`. `links` holds only URLs that exist, and each becomes a button. The home page boots once per document load (`bootedThisLoad` in `page.tsx`), so returning from a case study to `/#projects` lands on the section without replaying the boot. Links between the home page and case studies are `WarpLink` (`src/components/shared/PageTransition.tsx`), never a bare `next/link`. Blast doors close, the route changes behind them with `scroll: false`, and the doors open on a page already placed: a case study at the top, the home page at the exact offset the visitor left from (`orbital.return.y` in `sessionStorage`), falling back to the hash. Scroll placement is owned there because Lenis carries its position across client navigations, which is why a case study once opened at the bottom. Browser back and forward skip the doors but land the same way.
...
- **Contact Copy**: lives in `src/lib/data/contact.ts` (headline, intro, location, and `messengers`: Telegram and WhatsApp, shown as rows in the contact section's direct lines and returned by the agent's `get_contact`. The WhatsApp link is `wa.me/<number>` with no `?text=`, so it opens a chat and sends nothing), like every other section's content. It names no framework: the hero states the approach rather than the stack, and a contact line selling two tools contradicts it. The location is the real one and matches the hero clock's city.
- **Contact Form**: Netlify Forms. The form is declared in `public/__forms.html` and posted there by `fetch` from `Horizon.tsx`. Netlify registers a form by parsing the static files it deploys, and an App Router page is not one of those files, so the declaration file is what makes the form exist; every field the component sends must also appear in it. The React form itself carries no `data-netlify` or `netlify-honeypot`: Netlify's Next adapter scans prerendered pages for those attributes and fails the deploy when it finds them. Its `action` points at `/__forms.html` so a visitor without JS still submits to the right place. A failed post shows the email address instead of swallowing the message. Form detection has to be on in the Netlify site settings, and submissions land under Forms there.
...
- **Terminal Agent**: the nav terminal answers questions through ARCH, an agent behind `src/app/api/chat/route.ts` that calls Gemini through the `openai` SDK and Gemini's OpenAI-compatible base URL (`src/lib/chat/config.ts`). It needs `GEMINI_API_KEY` in the Netlify environment (and `.env.local` for dev); `GEMINI_MODEL` sets the model id and `GEMINI_REASONING_EFFORT` optionally caps thinking. Tool results carry absolute URLs (`SITE_URL`): handed a relative path, the model invented a domain for it. Never prefix either with `NEXT_PUBLIC_`. Four rules keep it cheap and correct:
  ...
  4. **The route trusts nothing from the client.** It rebuilds each message from known fields, rejects system messages, caps input, turns and history size (`CHAT_LIMITS`), and rate-limits per IP. The limiter is per function instance; move it to shared storage if the key is ever abused. Visitors never see a server error string: the route sends a `ChatErrorCode` and `explainError()` in `AdminTerminal.tsx` turns it into a plain sentence that says what happened and what to do. `busy` means the provider's quota or capacity (a 429 or 5xx from Gemini, or a gateway page), so the message asks them to try again in a minute. `rate_limited` means this visitor hit the per-IP limit.
